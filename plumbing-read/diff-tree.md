...
| Efecto principal | comparar los blobs y modos de dos árboles. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: comparar los blobs y modos de dos árboles.
...
| --- | --- | --- |
| Caso base | comparar los blobs y modos de dos árboles | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git diff-tree a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Salida para scripts | Producir registros con campos y separadores definidos. | Prueba nombres con espacios y saltos de línea. |
| Validación | Comprobar el resultado de git diff-tree con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| `--no-commit-id` | Desactiva el comportamiento `commit-id` para esta invocación. |
...
| `--combined-all-paths` | Incluye elementos adicionales dentro del alcance indicado. |
