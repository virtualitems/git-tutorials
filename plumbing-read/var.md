...
| Efecto principal | mostrar variables lógicas calculadas por Git. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: mostrar variables lógicas calculadas por Git.
...
| --- | --- | --- |
| Caso base | mostrar variables lógicas calculadas por Git | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git var a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git var con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
