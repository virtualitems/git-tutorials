...
| Efecto principal | detectar packs cuyo contenido ya está cubierto por otros packs. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: detectar packs cuyo contenido ya está cubierto por otros packs.
...
| --- | --- | --- |
| Caso base | detectar packs cuyo contenido ya está cubierto por otros packs | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git pack-redundant a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git pack-redundant con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| `--alt-odb` | Activa el modo `--alt-odb`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
