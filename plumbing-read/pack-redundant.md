...
git pack-redundant consulta objetos, referencias, índices, packs y relaciones entre commits. Recibe como entrada objetos, referencias, árboles o entradas del índice que debe leer la consulta. La operación consiste en detectar packs cuyo contenido ya está cubierto por otros packs.
...
La invocación `git pack-redundant --all` ejecuta esta operación: detectar packs cuyo contenido ya está cubierto por otros packs. Después, la salida estructurada puede compararse o pasar a otro proceso sin alterar el repositorio. Conserva stdout, stderr y el código de terminación cuando el ejemplo forme parte de un script.
...
detectar packs cuyo contenido ya está cubierto por otros packs. Usa el [ejemplo mínimo](#ejemplo-mínimo) como punto de partida. Ejecuta el ejemplo mínimo y registra el estado antes y después.

### Alcance explícito

Aplicar git pack-redundant a una referencia, rango o ruta identificada. Usa el [ejemplo mínimo](#ejemplo-mínimo) como punto de partida. Resuelve cada argumento antes de ejecutar y usa `--` para rutas.

### Validación

Comprobar el resultado de git pack-redundant con una orden de lectura independiente. Usa el [ejemplo mínimo](#ejemplo-mínimo) como punto de partida. No uses la misma salida como única prueba del cambio.
...
La opción no recibe un valor separado en la forma mostrada por la ayuda corta. Los argumentos que aparecen después pertenecen a `git pack-redundant` o a otra opción. El código de terminación distingue una ejecución aceptada de un error y, en algunos comandos de consulta, de una respuesta negativa. Ejecuta la comprobación inmediatamente después para que ningún comando intermedio cambie el estado que estás observando.
...
En `git pack-redundant`, alt odb modifica la forma en que se ejecuta detectar packs cuyo contenido ya está cubierto por otros packs. Mantén iguales los demás argumentos para atribuir el cambio observado a esta opción.
...
La opción limita o amplía el conjunto sobre el que se ejecuta detectar packs cuyo contenido ya está cubierto por otros packs. Comprueba la selección con una forma de lectura antes de combinarla con una opción que escriba estado.
