...
| Efecto principal | extraer el identificador incluido por git archive en un tar. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: extraer el identificador incluido por git archive en un tar.
...
| --- | --- | --- |
| Caso base | extraer el identificador incluido por git archive en un tar | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git get-tar-commit-id a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git get-tar-commit-id con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
