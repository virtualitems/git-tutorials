...
git repo consulta objetos, referencias, índices, packs y relaciones entre commits. Recibe como entrada objetos, referencias, árboles o entradas del índice que debe leer la consulta. La operación consiste en consultar propiedades y estructura del repositorio.
...
La invocación `git repo info --all` ejecuta esta operación: consultar propiedades y estructura del repositorio. Después, la salida estructurada puede compararse o pasar a otro proceso sin alterar el repositorio. Conserva stdout, stderr y el código de terminación cuando el ejemplo forme parte de un script.
...
consultar propiedades y estructura del repositorio. Usa el [ejemplo mínimo](#ejemplo-mínimo) como punto de partida. Ejecuta el ejemplo mínimo y registra el estado antes y después.

### Alcance explícito

Aplicar git repo a una referencia, rango o ruta identificada. Usa el [ejemplo mínimo](#ejemplo-mínimo) como punto de partida. Resuelve cada argumento antes de ejecutar y usa `--` para rutas.
...
Comprobar el resultado de git repo con una orden de lectura independiente. Usa el [ejemplo mínimo](#ejemplo-mínimo) como punto de partida. No uses la misma salida como única prueba del cambio.
...
La opción no recibe un valor separado en la forma mostrada por la ayuda corta. Los argumentos que aparecen después pertenecen a `git repo` o a otra opción. El código de terminación distingue una ejecución aceptada de un error y, en algunos comandos de consulta, de una respuesta negativa. Ejecuta la comprobación inmediatamente después para que ningún comando intermedio cambie el estado que estás observando.

### `-z`

Termina registros con NUL para evitar división por espacios o saltos de línea.

En `git repo`, z modifica la forma en que se ejecuta consultar propiedades y estructura del repositorio. Mantén iguales los demás argumentos para atribuir el cambio observado a esta opción.
...
La opción limita o amplía el conjunto sobre el que se ejecuta consultar propiedades y estructura del repositorio. Comprueba la selección con una forma de lectura antes de combinarla con una opción que escriba estado.
...
En `git repo`, keys modifica la forma en que se ejecuta consultar propiedades y estructura del repositorio. Mantén iguales los demás argumentos para atribuir el cambio observado a esta opción.
...
En `git repo`, ayuda modifica la forma en que se ejecuta consultar propiedades y estructura del repositorio. Mantén iguales los demás argumentos para atribuir el cambio observado a esta opción.
