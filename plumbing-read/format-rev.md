...
git format-rev consulta objetos, referencias, índices, packs y relaciones entre commits. Recibe como entrada objetos, referencias, árboles o entradas del índice que debe leer la consulta. La operación consiste en formatear revisiones recibidas por la entrada estándar.
...
La invocación `git format-rev` ejecuta esta operación: formatear revisiones recibidas por la entrada estándar. Después, la salida estructurada puede compararse o pasar a otro proceso sin alterar el repositorio. Conserva stdout, stderr y el código de terminación cuando el ejemplo forme parte de un script.
...
formatear revisiones recibidas por la entrada estándar. Usa el [ejemplo mínimo](#ejemplo-mínimo) como punto de partida. Ejecuta el ejemplo mínimo y registra el estado antes y después.

### Alcance explícito

Aplicar git format-rev a una referencia, rango o ruta identificada. Usa el [ejemplo mínimo](#ejemplo-mínimo) como punto de partida. Resuelve cada argumento antes de ejecutar y usa `--` para rutas.
...
Comprobar el resultado de git format-rev con una orden de lectura independiente. Usa el [ejemplo mínimo](#ejemplo-mínimo) como punto de partida. No uses la misma salida como única prueba del cambio.
...
La opción no recibe un valor separado en la forma mostrada por la ayuda corta. Los argumentos que aparecen después pertenecen a `git format-rev` o a otra opción. El código de terminación distingue una ejecución aceptada de un error y, en algunos comandos de consulta, de una respuesta negativa. Ejecuta la comprobación inmediatamente después para que ningún comando intermedio cambie el estado que estás observando.
...
En `git format-rev`, notas modifica la forma en que se ejecuta formatear revisiones recibidas por la entrada estándar. Mantén iguales los demás argumentos para atribuir el cambio observado a esta opción.
...
En `git format-rev`, z modifica la forma en que se ejecuta formatear revisiones recibidas por la entrada estándar. Mantén iguales los demás argumentos para atribuir el cambio observado a esta opción.
...
En `git format-rev`, ayuda modifica la forma en que se ejecuta formatear revisiones recibidas por la entrada estándar. Mantén iguales los demás argumentos para atribuir el cambio observado a esta opción.
