...
| Efecto principal | formatear revisiones recibidas por la entrada estándar. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: formatear revisiones recibidas por la entrada estándar.
...
| --- | --- | --- |
| Caso base | formatear revisiones recibidas por la entrada estándar | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git format-rev a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Salida para scripts | Producir registros con campos y separadores definidos. | Prueba nombres con espacios y saltos de línea. |
| Validación | Comprobar el resultado de git format-rev con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| --- | --- |
| `--stdin-mode` | Activa el modo `--stdin-mode`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
...
| `--null-output` | Selecciona un archivo de entrada o salida según la posición indicada en la sintaxis. |
| `--null-input` | Activa el modo `--null-input`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
