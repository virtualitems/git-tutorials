...
| Efecto principal | reescribir ramas mediante filtros sobre cada commit. | Comprueba el resultado con una orden de lectura. |
| Persistencia | Puede persistir el estado implicado por esta operación: reescribir ramas mediante filtros sobre cada commit. Las opciones pueden limitar o ampliar ese efecto. | Compara el estado antes y después. |
...
- La operación observable es: reescribir ramas mediante filtros sobre cada commit.
...
| --- | --- | --- |
| Caso base | reescribir ramas mediante filtros sobre cada commit | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git filter-branch a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git filter-branch con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| --- | --- |
| `--setup` | Activa el modo `--setup`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--subdirectory-filter` | Activa el modo `--subdirectory-filter`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--env-filter` | Activa el modo `--env-filter`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--tree-filter` | Activa el modo `--tree-filter`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--index-filter` | Activa el modo `--index-filter`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--parent-filter` | Activa el modo `--parent-filter`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--msg-filter` | Activa el modo `--msg-filter`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--commit-filter` | Activa el modo `--commit-filter`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
