...
| Efecto principal | usar una interfaz gráfica para preparar cambios y crear commits. | Comprueba el resultado con una orden de lectura. |
| Persistencia | Puede persistir el estado implicado por esta operación: usar una interfaz gráfica para preparar cambios y crear commits. Las opciones pueden limitar o ampliar ese efecto. | Compara el estado antes y después. |
...
- La operación observable es: usar una interfaz gráfica para preparar cambios y crear commits.
...
| --- | --- | --- |
| Caso base | usar una interfaz gráfica para preparar cambios y crear commits | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git gui a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git gui con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
