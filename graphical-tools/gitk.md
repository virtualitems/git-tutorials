...
| Efecto principal | explorar el historial y sus relaciones en una interfaz gráfica. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: explorar el historial y sus relaciones en una interfaz gráfica.
...
| --- | --- | --- |
| Caso base | explorar el historial y sus relaciones en una interfaz gráfica | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar gitk a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de gitk con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
