...
| Efecto principal | nombrar un commit con la referencia cercana que lo alcanza. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: nombrar un commit con la referencia cercana que lo alcanza.
...
| --- | --- | --- |
| Caso base | nombrar un commit con la referencia cercana que lo alcanza | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git describe a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git describe con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| `--dirty` | Activa el modo `--dirty`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
...
| `--exact-match` | Activa el modo `--exact-match`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--candidates` | Activa el modo `--candidates`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--match` | Activa el modo `--match`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
...
| `--broken` | Activa el modo `--broken`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
