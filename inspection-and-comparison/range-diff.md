...
| Efecto principal | comparar dos versiones de una serie de commits. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: comparar dos versiones de una serie de commits.
...
| --- | --- | --- |
| Caso base | comparar dos versiones de una serie de commits | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git range-diff a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Salida para scripts | Producir registros con campos y separadores definidos. | Prueba nombres con espacios y saltos de línea. |
| Validación | Comprobar el resultado de git range-diff con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| `--no-dual-color` | Desactiva el comportamiento `dual-color` para esta invocación. |
...
| `--left-only` | Activa el modo `--left-only`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--right-only` | Activa el modo `--right-only`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--diff-merges` | Activa el modo `--diff-merges`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--remerge-diff` | Activa el modo `--remerge-diff`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--no-notes` | Desactiva el comportamiento `notes` para esta invocación. |
| `--notes` | Activa el modo `--notes`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--dual-color` | Activa el modo `--dual-color`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
