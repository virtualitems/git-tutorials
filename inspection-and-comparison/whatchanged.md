...
| Efecto principal | mostrar commits junto con diferencias en formato sin procesar. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: mostrar commits junto con diferencias en formato sin procesar.
...
| --- | --- | --- |
| Caso base | mostrar commits junto con diferencias en formato sin procesar | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git whatchanged a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git whatchanged con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
