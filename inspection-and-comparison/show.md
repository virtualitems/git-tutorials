...
| Efecto principal | mostrar un objeto y la información asociada a su tipo. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: mostrar un objeto y la información asociada a su tipo.
...
| --- | --- | --- |
| Caso base | mostrar un objeto y la información asociada a su tipo | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git show a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git show con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
