...
| Efecto principal | mostrar el commit que modificó por última vez cada ruta. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: mostrar el commit que modificó por última vez cada ruta.
...
| --- | --- | --- |
| Caso base | mostrar el commit que modificó por última vez cada ruta | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git last-modified a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Salida para scripts | Producir registros con campos y separadores definidos. | Prueba nombres con espacios y saltos de línea. |
| Validación | Comprobar el resultado de git last-modified con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| `--show-trees` | Incluye información adicional en la salida. |
