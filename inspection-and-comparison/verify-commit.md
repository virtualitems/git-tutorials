...
| Efecto principal | verificar la firma criptográfica de commits. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: verificar la firma criptográfica de commits.
...
| --- | --- | --- |
| Caso base | verificar la firma criptográfica de commits | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git verify-commit a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git verify-commit con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
