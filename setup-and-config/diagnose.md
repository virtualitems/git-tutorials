...
| Efecto principal | generar un archivo con datos de diagnóstico del repositorio. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: generar un archivo con datos de diagnóstico del repositorio.
...
| --- | --- | --- |
| Caso base | generar un archivo con datos de diagnóstico del repositorio | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git diagnose a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git diagnose con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
