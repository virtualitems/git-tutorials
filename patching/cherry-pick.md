...
| Efecto principal | aplicar en la rama actual el cambio de commits existentes. | Comprueba el resultado con una orden de lectura. |
| Persistencia | Puede persistir el estado implicado por esta operación: aplicar en la rama actual el cambio de commits existentes. Las opciones pueden limitar o ampliar ese efecto. | Compara el estado antes y después. |
...
- La operación observable es: aplicar en la rama actual el cambio de commits existentes.
...
| --- | --- | --- |
| Caso base | aplicar en la rama actual el cambio de commits existentes | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git cherry-pick a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
...
| Validación | Comprobar el resultado de git cherry-pick con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| `--keep-redundant-commits` | Activa el modo `--keep-redundant-commits`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
