...
| Efecto principal | identificar los archivos y directorios internos de un repositorio. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: identificar los archivos y directorios internos de un repositorio.
...
| --- | --- | --- |
| Caso base | identificar los archivos y directorios internos de un repositorio | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| directorio Git | Aplicar las reglas de directorio Git. | Cambia una entrada y comprueba el efecto que define la guía. |
| objetos y packs | Aplicar las reglas de objetos y packs. | Cambia una entrada y comprueba el efecto que define la guía. |
| referencias y reflogs | Aplicar las reglas de referencias y reflogs. | Cambia una entrada y comprueba el efecto que define la guía. |
| índice | Aplicar las reglas de índice. | Cambia una entrada y comprueba el efecto que define la guía. |
| archivos de estado transitorio | Aplicar las reglas de archivos de estado transitorio. | Cambia una entrada y comprueba el efecto que define la guía. |

## Reglas por área

| Área | Regla | Comprobación reproducible |
| --- | --- | --- |
| Objetos | `objects` guarda objetos sueltos, packs y alternates. | Usa comandos de plomería; no edites los archivos. |
| Referencias | `refs` y `packed-refs` representan nombres; reflogs registran movimientos cuando están activos. | Compara `show-ref` y `reflog`. |
| Índice | `index` representa el próximo tree y puede contener etapas de conflicto. | Ejecuta `ls-files --stage`. |
| Estado transitorio | Archivos como `MERGE_HEAD` o directorios de rebase describen una operación pausada. | Usa `git status` para elegir continuación o aborto. |
| Worktrees | Repositorios con worktrees separan datos comunes de datos por worktree. | Consulta `git rev-parse --git-common-dir --git-path HEAD`. |
