...
| Efecto principal | ejecutar programas en puntos definidos del flujo de Git. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: ejecutar programas en puntos definidos del flujo de Git.
...
| --- | --- | --- |
| Caso base | ejecutar programas en puntos definidos del flujo de Git | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| ubicación y descubrimiento | Aplicar las reglas de ubicación y descubrimiento. | Cambia una entrada y comprueba el efecto que define la guía. |
| entrada por argumentos o stdin | Aplicar las reglas de entrada por argumentos o stdin. | Cambia una entrada y comprueba el efecto que define la guía. |
| entorno | Aplicar las reglas de entorno. | Cambia una entrada y comprueba el efecto que define la guía. |
| códigos de salida | Aplicar las reglas de códigos de salida. | Cambia una entrada y comprueba el efecto que define la guía. |
| despliegue y pruebas | Aplicar las reglas de despliegue y pruebas. | Cambia una entrada y comprueba el efecto que define la guía. |

## Reglas por área

| Área | Regla | Comprobación reproducible |
| --- | --- | --- |
| Descubrimiento | Git busca hooks en `core.hooksPath` o en la ruta de hooks del repositorio. | Consulta `git config --show-origin core.hooksPath`. |
| Ejecución | El archivo debe poder ejecutarse y usar un intérprete disponible. | Invócalo con el mismo usuario que ejecuta Git. |
| Entrada | Cada hook define argumentos, stdin y variables; no comparten un contrato único. | Registra entradas en un repositorio de prueba. |
| Salida | Un código distinto de cero puede rechazar la operación cuando el hook lo define. | Prueba códigos 0 y 1 y observa la orden que lo llama. |
| Distribución | Los hooks no viajan con un clone por defecto. | Versiona una fuente y configura su instalación de forma explícita. |
