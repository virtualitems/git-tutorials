...
| Efecto principal | entender el modelo de repositorios anidados como submódulos. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: entender el modelo de repositorios anidados como submódulos.
...
| --- | --- | --- |
| Caso base | entender el modelo de repositorios anidados como submódulos | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| gitlink y `.gitmodules` | Aplicar las reglas de gitlink y `.gitmodules`. | Cambia una entrada y comprueba el efecto que define la guía. |
| inicialización | Aplicar las reglas de inicialización. | Cambia una entrada y comprueba el efecto que define la guía. |
| actualización | Aplicar las reglas de actualización. | Cambia una entrada y comprueba el efecto que define la guía. |
| cambios locales | Aplicar las reglas de cambios locales. | Cambia una entrada y comprueba el efecto que define la guía. |
| recursión y seguridad | Aplicar las reglas de recursión y seguridad. | Cambia una entrada y comprueba el efecto que define la guía. |

## Reglas por área

| Área | Regla | Comprobación reproducible |
| --- | --- | --- |
| Gitlink | El superproyecto registra el commit del submódulo como una entrada de modo 160000. | Usa `git ls-tree HEAD ruta`. |
| Declaración | `.gitmodules` aporta ruta, URL y configuración compartida. | Consulta el archivo con `git config -f`. |
| Inicialización | Init copia configuración permitida a la configuración local. | Compara valores antes y después. |
| Actualización | Update materializa el commit del gitlink, no cualquier tip remoto. | Compara `rev-parse HEAD` dentro del submódulo con `ls-tree`. |
| Recursión | Las opciones recurse-submodules propagan operaciones bajo reglas de cada comando. | Prueba un submódulo con cambios locales. |
