...
| Efecto principal | configurar la obtención y el almacenamiento de credenciales. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: configurar la obtención y el almacenamiento de credenciales.
...
| --- | --- | --- |
| Caso base | configurar la obtención y el almacenamiento de credenciales | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| contexto de credencial | Aplicar las reglas de contexto de credencial. | Cambia una entrada y comprueba el efecto que define la guía. |
| acciones `fill`, `approve` y `reject` | Aplicar las reglas de acciones `fill`, `approve` y `reject`. | Cambia una entrada y comprueba el efecto que define la guía. |
| cascada de helpers | Aplicar las reglas de cascada de helpers. | Cambia una entrada y comprueba el efecto que define la guía. |
| entrada y salida por pares clave-valor | Aplicar las reglas de entrada y salida por pares clave-valor. | Cambia una entrada y comprueba el efecto que define la guía. |
| protección del secreto | Aplicar las reglas de protección del secreto. | Cambia una entrada y comprueba el efecto que define la guía. |

## Reglas por área

| Área | Regla | Comprobación reproducible |
| --- | --- | --- |
| Contexto | El protocolo describe URL mediante campos como protocol, host, path y username. | Envía el contexto a `git credential fill` sin registrar el secreto. |
| Fill | `fill` consulta configuración y helpers hasta completar una credencial o fallar. | Captura los campos devueltos y elimina el password del registro. |
| Approve | `approve` notifica a los helpers que una credencial funcionó. | Usa un helper de laboratorio y comprueba su almacenamiento. |
| Reject | `reject` solicita retirar una credencial que falló. | Consulta de nuevo el helper y confirma que no devuelve la entrada. |
| Terminación | Cada solicitud termina con una línea vacía. | Prueba el protocolo con `printf` y confirma que el proceso no queda esperando. |
