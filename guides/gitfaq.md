...
| Efecto principal | resolver preguntas sobre configuración, historial y archivos. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: resolver preguntas sobre configuración, historial y archivos.
...
| --- | --- | --- |
| Caso base | resolver preguntas sobre configuración, historial y archivos | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| recuperación | Aplicar las reglas de recuperación. | Cambia una entrada y comprueba el efecto que define la guía. |
| configuración | Aplicar las reglas de configuración. | Cambia una entrada y comprueba el efecto que define la guía. |
| rutas y nombres | Aplicar las reglas de rutas y nombres. | Cambia una entrada y comprueba el efecto que define la guía. |
| credenciales | Aplicar las reglas de credenciales. | Cambia una entrada y comprueba el efecto que define la guía. |
| interoperabilidad | Aplicar las reglas de interoperabilidad. | Cambia una entrada y comprueba el efecto que define la guía. |

## Reglas por área

| Área | Regla | Comprobación reproducible |
| --- | --- | --- |
| Recuperación | El reflog permite localizar valores anteriores de referencias mientras no hayan vencido. | Busca el hash y crea una rama antes de mantenimiento. |
| Configuración | `--show-origin` identifica qué archivo aportó un valor. | Consulta todas las apariciones de la clave. |
| Rutas | Git registra rutas y el shell decide la expansión inicial de argumentos. | Usa `--` y una ruta con espacios. |
| Credenciales | Los helpers reciben contexto y secreto por un protocolo separado de la URL. | Evita secretos en argumentos y remotos. |
| Historial | Cambiar un commit cambia sus descendientes porque el identificador incluye el padre. | Compara hashes antes y después de un rebase. |
