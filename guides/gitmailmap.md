...
| Efecto principal | unificar nombres y correos que representan a una misma persona. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: unificar nombres y correos que representan a una misma persona.
...
| --- | --- | --- |
| Caso base | unificar nombres y correos que representan a una misma persona | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| formatos de entrada | Aplicar las reglas de formatos de entrada. | Cambia una entrada y comprueba el efecto que define la guía. |
| correo canónico | Aplicar las reglas de correo canónico. | Cambia una entrada y comprueba el efecto que define la guía. |
| nombre canónico | Aplicar las reglas de nombre canónico. | Cambia una entrada y comprueba el efecto que define la guía. |
| precedencia | Aplicar las reglas de precedencia. | Cambia una entrada y comprueba el efecto que define la guía. |
| consultas y logs | Aplicar las reglas de consultas y logs. | Cambia una entrada y comprueba el efecto que define la guía. |

## Reglas por área

| Área | Regla | Comprobación reproducible |
| --- | --- | --- |
| Identidad | Una entrada asocia nombre o correo observado con una identidad canónica. | Ejecuta `git check-mailmap` con la identidad original. |
| Formas | El formato permite corregir nombre, correo o ambos según los campos presentes. | Prueba cada forma sobre datos de laboratorio. |
| Archivos | Git puede leer `.mailmap` y ubicaciones configuradas. | Consulta la configuración y ejecuta desde otra ruta. |
| Historial | La mailmap cambia presentación, no reescribe commits. | Compara `git cat-file -p` con `git log --use-mailmap`. |
| Automatización | Los formatos de log pueden aplicar mailmap a campos concretos. | Declara el placeholder y valida el correo emitido. |
