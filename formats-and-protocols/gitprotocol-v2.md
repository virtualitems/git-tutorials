...
| Efecto principal | seguir comandos y capacidades en la versión 2 del protocolo. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: seguir comandos y capacidades en la versión 2 del protocolo.
...
| --- | --- | --- |
| Caso base | seguir comandos y capacidades en la versión 2 del protocolo | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| negociación de versión | Aplicar las reglas de negociación de versión. | Cambia una entrada y comprueba el efecto que define la guía. |
| anuncio de capacidades | Aplicar las reglas de anuncio de capacidades. | Cambia una entrada y comprueba el efecto que define la guía. |
| `ls-refs` | Aplicar las reglas de `ls-refs`. | Cambia una entrada y comprueba el efecto que define la guía. |
| `fetch` | Aplicar las reglas de `fetch`. | Cambia una entrada y comprueba el efecto que define la guía. |
| delimitación de secciones | Aplicar las reglas de delimitación de secciones. | Cambia una entrada y comprueba el efecto que define la guía. |

## Reglas por área

| Área | Regla | Comprobación reproducible |
| --- | --- | --- |
| Versión | El cliente solicita versión 2 por el mecanismo del transporte. | Confirma que la primera respuesta declara version 2. |
| Capacidades | El servidor anuncia comandos y capacidades como pkt-lines. | No envíes un comando que no fue anunciado. |
| ls-refs | `ls-refs` solicita referencias con prefijos y atributos opcionales. | Termina argumentos con flush y valida cada respuesta. |
| fetch | `fetch` negocia wants, haves, shallow, filtros y packfile por secciones. | Respeta delimitadores entre acknowledgments y packfile. |
| Estado | Cada comando forma una solicitud delimitada; la conexión puede atender más de una. | Restablece el parser al terminar la respuesta. |
