...
| Efecto principal | interpretar la cabecera, prerrequisitos y referencias de un bundle. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: interpretar la cabecera, prerrequisitos y referencias de un bundle.
...
| --- | --- | --- |
| Caso base | interpretar la cabecera, prerrequisitos y referencias de un bundle | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| firma de cabecera | Aplicar las reglas de firma de cabecera. | Cambia una entrada y comprueba el efecto que define la guía. |
| prerrequisitos | Aplicar las reglas de prerrequisitos. | Cambia una entrada y comprueba el efecto que define la guía. |
| referencias | Aplicar las reglas de referencias. | Cambia una entrada y comprueba el efecto que define la guía. |
| pack embebido | Aplicar las reglas de pack embebido. | Cambia una entrada y comprueba el efecto que define la guía. |
| validación | Aplicar las reglas de validación. | Cambia una entrada y comprueba el efecto que define la guía. |

## Reglas por área

| Área | Regla | Comprobación reproducible |
| --- | --- | --- |
| Cabecera | El archivo comienza con una firma de versión de bundle. | Lee la primera línea y rechaza otra firma. |
| Prerrequisitos | Las líneas con OID negativo declaran objetos que el receptor debe poseer. | Ejecuta `git bundle verify`. |
| Referencias | Las líneas con OID positivo anuncian tips que el bundle transporta. | Lista con `git bundle list-heads`. |
| Pack | Después de la línea vacía aparece un packfile. | Valida el pack con herramientas de índice y fsck. |
| Versión | La versión del formato cambia qué capacidades o filtros puede representar. | Comprueba compatibilidad antes de crear. |
