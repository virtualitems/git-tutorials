...
| Efecto principal | interpretar objetos, deltas e índices de archivos pack. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: interpretar objetos, deltas e índices de archivos pack.
...
| --- | --- | --- |
| Caso base | interpretar objetos, deltas e índices de archivos pack | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| cabecera | Aplicar las reglas de cabecera. | Cambia una entrada y comprueba el efecto que define la guía. |
| representación de objetos | Aplicar las reglas de representación de objetos. | Cambia una entrada y comprueba el efecto que define la guía. |
| deltas | Aplicar las reglas de deltas. | Cambia una entrada y comprueba el efecto que define la guía. |
| tráiler | Aplicar las reglas de tráiler. | Cambia una entrada y comprueba el efecto que define la guía. |
| índice asociado | Aplicar las reglas de índice asociado. | Cambia una entrada y comprueba el efecto que define la guía. |

## Reglas por área

| Área | Regla | Comprobación reproducible |
| --- | --- | --- |
| Cabecera | PACK, versión y número de objetos abren el archivo. | Valida antes de recorrer objetos. |
| Objeto | Cada entrada declara tipo y tamaño mediante enteros de longitud variable. | Rechaza entradas que excedan el archivo. |
| Delta | OFS_DELTA usa distancia dentro del pack y REF_DELTA usa OID base. | Resuelve la base antes de aplicar instrucciones. |
| Tráiler | El hash final cubre el contenido anterior del pack. | Compara con `index-pack`. |
| Índice | El índice externo relaciona OID con offsets y checksums. | Ejecuta `verify-pack` sobre el índice. |
