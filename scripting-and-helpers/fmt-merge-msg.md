...
| Efecto principal | generar el mensaje de un commit de fusión. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: generar el mensaje de un commit de fusión.
...
| --- | --- | --- |
| Caso base | generar el mensaje de un commit de fusión | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git fmt-merge-msg a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git fmt-merge-msg con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| `--no-log` | Desactiva el comportamiento `log` para esta invocación. |
