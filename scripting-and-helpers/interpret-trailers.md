...
| Efecto principal | analizar y añadir campos al final de mensajes de commit. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: analizar y añadir campos al final de mensajes de commit.
...
| --- | --- | --- |
| Caso base | analizar y añadir campos al final de mensajes de commit | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git interpret-trailers a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git interpret-trailers con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| --- | --- |
| `--in-place` | Activa el modo `--in-place`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--trim-empty` | Activa el modo `--trim-empty`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--trailer` | Activa el modo `--trailer`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--parse` | Activa el modo `--parse`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--where` | Activa el modo `--where`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--if-exists` | Activa el modo `--if-exists`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--if-missing` | Activa el modo `--if-missing`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--only-trailers` | Activa el modo `--only-trailers`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--only-input` | Activa el modo `--only-input`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--unfold` | Activa el modo `--unfold`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--no-divider` | Desactiva el comportamiento `divider` para esta invocación. |
| `--divider` | Activa el modo `--divider`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
