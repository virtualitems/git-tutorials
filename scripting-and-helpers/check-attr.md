...
| Efecto principal | consultar los atributos que se aplican a una ruta. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: consultar los atributos que se aplican a una ruta.
...
| --- | --- | --- |
| Caso base | consultar los atributos que se aplican a una ruta | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git check-attr a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Salida para scripts | Producir registros con campos y separadores definidos. | Prueba nombres con espacios y saltos de línea. |
| Validación | Comprobar el resultado de git check-attr con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
