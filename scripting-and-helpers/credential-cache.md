...
| Efecto principal | mantener credenciales durante un tiempo en memoria. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: mantener credenciales durante un tiempo en memoria.
...
| --- | --- | --- |
| Caso base | mantener credenciales durante un tiempo en memoria | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git credential-cache a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git credential-cache con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| `--socket` | Activa el modo `--socket`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
