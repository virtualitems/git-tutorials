...
| Efecto principal | guardar credenciales sin cifrado en un archivo. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: guardar credenciales sin cifrado en un archivo.
...
| --- | --- | --- |
| Caso base | guardar credenciales sin cifrado en un archivo | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git credential-store a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git credential-store con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
