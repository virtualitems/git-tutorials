...
| Efecto principal | validar la sintaxis de un nombre de referencia. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: validar la sintaxis de un nombre de referencia.
...
| --- | --- | --- |
| Caso base | validar la sintaxis de un nombre de referencia | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git check-ref-format a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git check-ref-format con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| --- | --- |
| `--normalize` | Activa el modo `--normalize`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--allow-onelevel` | Activa el modo `--allow-onelevel`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--refspec-pattern` | Activa el modo `--refspec-pattern`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
