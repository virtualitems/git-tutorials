...
| Efecto principal | dividir un buzón mbox o Maildir en mensajes. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: dividir un buzón mbox o Maildir en mensajes.
...
| --- | --- | --- |
| Caso base | dividir un buzón mbox o Maildir en mensajes | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git mailsplit a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git mailsplit con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| `--mboxrd` | Activa el modo `--mboxrd`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
