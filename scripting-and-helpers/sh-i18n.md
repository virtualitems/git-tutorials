...
| Efecto principal | cargar funciones de internacionalización en scripts de shell. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: cargar funciones de internacionalización en scripts de shell.
...
| --- | --- | --- |
| Caso base | cargar funciones de internacionalización en scripts de shell | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git sh-i18n a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git sh-i18n con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
