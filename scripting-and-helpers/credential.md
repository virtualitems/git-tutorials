...
| Efecto principal | intercambiar credenciales con los ayudantes configurados. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: intercambiar credenciales con los ayudantes configurados.
...
| --- | --- | --- |
| Caso base | intercambiar credenciales con los ayudantes configurados | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git credential a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git credential con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
