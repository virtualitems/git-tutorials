...
| Efecto principal | convertir identidades mediante las reglas de mailmap. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: convertir identidades mediante las reglas de mailmap.
...
| --- | --- | --- |
| Caso base | convertir identidades mediante las reglas de mailmap | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git check-mailmap a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git check-mailmap con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| `--mailmap-file` | Selecciona un archivo de entrada o salida según la posición indicada en la sintaxis. |
| `--mailmap-blob` | Activa el modo `--mailmap-blob`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
