...
| Efecto principal | extraer componentes de una URL aceptada por Git. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: extraer componentes de una URL aceptada por Git.
...
| --- | --- | --- |
| Caso base | extraer componentes de una URL aceptada por Git | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git url-parse a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git url-parse con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
