...
canonical_file: "plumbing-write/update-index.md"
status: "expanded"
---

# `git update-index`

> Esta ruta conserva la clasificación `plumbing-commands`. La guía canónica está en [`plumbing-write/update-index.md`](../plumbing-write/update-index.md). Ambas documentan la misma URL del sitemap.
...
| Efecto principal | modificar directamente entradas y bits del índice. | Comprueba el resultado con una orden de lectura. |
| Persistencia | Puede persistir el estado implicado por esta operación: modificar directamente entradas y bits del índice. Las opciones pueden limitar o ampliar ese efecto. | Compara el estado antes y después. |
...
- La operación observable es: modificar directamente entradas y bits del índice.
...
| --- | --- | --- |
| Caso base | modificar directamente entradas y bits del índice | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git update-index a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Salida para scripts | Producir registros con campos y separadores definidos. | Prueba nombres con espacios y saltos de línea. |
| Validación | Comprobar el resultado de git update-index con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| `--remove` | Retira elementos según las condiciones de la orden. |
| `--force-remove` | Retira elementos según las condiciones de la orden. |
| `--replace` | Activa el modo `--replace`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
...
| `--cacheinfo` | Activa el modo `--cacheinfo`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--ignore-submodules` | Excluye elementos que cumplan la condición indicada. |
| `--really-refresh` | Activa el modo `--really-refresh`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--chmod` | Cambia el bit ejecutable registrado en el índice, no el permiso del archivo en disco. |
| `--assume-unchanged` | Activa el modo `--assume-unchanged`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--no-assume-unchanged` | Desactiva el comportamiento `assume-unchanged` para esta invocación. |
| `--skip-worktree` | Activa el modo `--skip-worktree`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--no-skip-worktree` | Desactiva el comportamiento `skip-worktree` para esta invocación. |
| `--ignore-skip-worktree-entries` | Excluye elementos que cumplan la condición indicada. |
| `--info-only` | Activa el modo `--info-only`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
...
| `--index-info` | Activa el modo `--index-info`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--unresolve` | Activa el modo `--unresolve`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `-g` | Activa el modo `-g`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--again` | Activa el modo `--again`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--verbose` | Aumenta el detalle enviado a la salida. |
| `--clear-resolve-undo` | Activa el modo `--clear-resolve-undo`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--index-version` | Activa el modo `--index-version`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--show-index-version` | Incluye información adicional en la salida. |
| `--split-index` | Activa el modo `--split-index`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--untracked-cache` | Activa el modo `--untracked-cache`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--test-untracked-cache` | Activa el modo `--test-untracked-cache`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--force-untracked-cache` | Omite una protección concreta de la orden; requiere verificar origen y destino. |
| `--force-write-index` | Permite crear o escribir el elemento seleccionado. |
| `--fsmonitor` | Activa el modo `--fsmonitor`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--fsmonitor-valid` | Activa el modo `--fsmonitor-valid`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--no-fsmonitor-valid` | Desactiva el comportamiento `fsmonitor-valid` para esta invocación. |
