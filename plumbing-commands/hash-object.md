...
canonical_file: "plumbing-write/hash-object.md"
status: "expanded"
---

# `git hash-object`

> Esta ruta conserva la clasificación `plumbing-commands`. La guía canónica está en [`plumbing-write/hash-object.md`](../plumbing-write/hash-object.md). Ambas documentan la misma URL del sitemap.
...
| Efecto principal | calcular el identificador de un objeto y guardarlo si se solicita. | Comprueba el resultado con una orden de lectura. |
| Persistencia | Puede persistir el estado implicado por esta operación: calcular el identificador de un objeto y guardarlo si se solicita. Las opciones pueden limitar o ampliar ese efecto. | Compara el estado antes y después. |
...
- La operación observable es: calcular el identificador de un objeto y guardarlo si se solicita.
...
| --- | --- | --- |
| Caso base | calcular el identificador de un objeto y guardarlo si se solicita | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git hash-object a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git hash-object con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| `--no-filters` | Desactiva el comportamiento `filters` para esta invocación. |
| `--stdin` | Lee registros o nombres desde la entrada estándar. |
| `--literally` | Activa el modo `--literally`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--stdin-paths` | Activa el modo `--stdin-paths`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
