...
canonical_file: "plumbing-write/read-tree.md"
status: "expanded"
---

# `git read-tree`

> Esta ruta conserva la clasificación `plumbing-commands`. La guía canónica está en [`plumbing-write/read-tree.md`](../plumbing-write/read-tree.md). Ambas documentan la misma URL del sitemap.
...
| Efecto principal | cargar información de árboles en el índice. | Comprueba el resultado con una orden de lectura. |
| Persistencia | Puede persistir el estado implicado por esta operación: cargar información de árboles en el índice. Las opciones pueden limitar o ampliar ese efecto. | Compara el estado antes y después. |
...
- La operación observable es: cargar información de árboles en el índice.
...
| --- | --- | --- |
| Caso base | cargar información de árboles en el índice | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git read-tree a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Simulación | Calcular el efecto sin escribir el estado principal. | Compara la simulación con la selección prevista. |
| Validación | Comprobar el resultado de git read-tree con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| `--trivial` | Activa el modo `--trivial`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--aggressive` | Activa el modo `--aggressive`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--reset` | Activa el modo `--reset`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
...
| `--index-output` | Selecciona un archivo de entrada o salida según la posición indicada en la sintaxis. |
| `--no-sparse-checkout` | Desactiva el comportamiento `sparse-checkout` para esta invocación. |
...
| `--sparse-checkout` | Activa el modo `--sparse-checkout`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--debug-unpack` | Activa el modo `--debug-unpack`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
