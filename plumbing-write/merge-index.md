...
| Efecto principal | ejecutar un programa de fusión para entradas sin resolver. | Comprueba el resultado con una orden de lectura. |
| Persistencia | Puede persistir el estado implicado por esta operación: ejecutar un programa de fusión para entradas sin resolver. Las opciones pueden limitar o ampliar ese efecto. | Compara el estado antes y después. |
...
- La operación observable es: ejecutar un programa de fusión para entradas sin resolver.
...
| --- | --- | --- |
| Caso base | ejecutar un programa de fusión para entradas sin resolver | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git merge-index a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git merge-index con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
