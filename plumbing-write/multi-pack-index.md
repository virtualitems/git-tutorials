...
| Efecto principal | administrar un índice que cubre varios archivos pack. | Comprueba el resultado con una orden de lectura. |
| Persistencia | Puede persistir el estado implicado por esta operación: administrar un índice que cubre varios archivos pack. Las opciones pueden limitar o ampliar ese efecto. | Compara el estado antes y después. |
...
- La operación observable es: administrar un índice que cubre varios archivos pack.
...
| --- | --- | --- |
| Caso base | administrar un índice que cubre varios archivos pack | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git multi-pack-index a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git multi-pack-index con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| --- | --- |
| `--preferred-pack` | Activa el modo `--preferred-pack`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
| `--bitmap` | Activa el modo `--bitmap`; los argumentos y restricciones aparecen en la sintaxis y en la fuente oficial. |
...
| `--refs-snapshot` | Selecciona o modifica referencias dentro del alcance de la orden. |
| `--write-chain-file` | Selecciona un archivo de entrada o salida según la posición indicada en la sintaxis. |
