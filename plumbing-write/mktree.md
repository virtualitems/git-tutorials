...
| Efecto principal | crear un objeto árbol a partir de una lista de entradas. | Comprueba el resultado con una orden de lectura. |
| Persistencia | Puede persistir el estado implicado por esta operación: crear un objeto árbol a partir de una lista de entradas. Las opciones pueden limitar o ampliar ese efecto. | Compara el estado antes y después. |
...
- La operación observable es: crear un objeto árbol a partir de una lista de entradas.
...
| --- | --- | --- |
| Caso base | crear un objeto árbol a partir de una lista de entradas | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git mktree a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Salida para scripts | Producir registros con campos y separadores definidos. | Prueba nombres con espacios y saltos de línea. |
| Validación | Comprobar el resultado de git mktree con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
