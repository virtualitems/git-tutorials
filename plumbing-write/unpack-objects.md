...
| Efecto principal | extraer objetos de un flujo pack. | Comprueba el resultado con una orden de lectura. |
| Persistencia | Puede persistir el estado implicado por esta operación: extraer objetos de un flujo pack. Las opciones pueden limitar o ampliar ese efecto. | Compara el estado antes y después. |
...
- La operación observable es: extraer objetos de un flujo pack.
...
| --- | --- | --- |
| Caso base | extraer objetos de un flujo pack | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git unpack-objects a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git unpack-objects con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
