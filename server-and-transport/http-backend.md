...
| Efecto principal | atender operaciones Git del lado servidor sobre HTTP. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: atender operaciones Git del lado servidor sobre HTTP.
...
| --- | --- | --- |
| Caso base | atender operaciones Git del lado servidor sobre HTTP | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git http-backend a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git http-backend con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
