...
| Efecto principal | negociar y enviar objetos a un cliente de fetch. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: negociar y enviar objetos a un cliente de fetch.
...
| --- | --- | --- |
| Caso base | negociar y enviar objetos a un cliente de fetch | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git upload-pack a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git upload-pack con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
...
| `--advertise-refs` | Selecciona o modifica referencias dentro del alcance de la orden. |
