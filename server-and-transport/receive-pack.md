...
| Efecto principal | recibir objetos y solicitudes de actualización de referencias. | Comprueba el resultado con una orden de lectura. |
| Persistencia | Puede persistir el estado implicado por esta operación: recibir objetos y solicitudes de actualización de referencias. Las opciones pueden limitar o ampliar ese efecto. | Compara el estado antes y después. |
...
- La operación observable es: recibir objetos y solicitudes de actualización de referencias.
...
| --- | --- | --- |
| Caso base | recibir objetos y solicitudes de actualización de referencias | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git receive-pack a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git receive-pack con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
