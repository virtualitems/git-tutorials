...
| Efecto principal | responder a una solicitud remota de git archive. | Comprueba el resultado con una orden de lectura. |
...
- La operación observable es: responder a una solicitud remota de git archive.
...
| --- | --- | --- |
| Caso base | responder a una solicitud remota de git archive | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git upload-archive a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git upload-archive con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
