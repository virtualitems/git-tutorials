...
| Efecto principal | permitir que clientes CVS accedan a un repositorio Git. | Comprueba el resultado con una orden de lectura. |
| Persistencia | Puede persistir el estado implicado por esta operación: permitir que clientes CVS accedan a un repositorio Git. Las opciones pueden limitar o ampliar ese efecto. | Compara el estado antes y después. |
...
- La operación observable es: permitir que clientes CVS accedan a un repositorio Git.
...
| --- | --- | --- |
| Caso base | permitir que clientes CVS accedan a un repositorio Git | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git cvsserver a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git cvsserver con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
