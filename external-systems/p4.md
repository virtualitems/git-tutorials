...
| Efecto principal | importar desde Perforce y enviar cambios de vuelta. | Comprueba el resultado con una orden de lectura. |
| Persistencia | Puede persistir el estado implicado por esta operación: importar desde Perforce y enviar cambios de vuelta. Las opciones pueden limitar o ampliar ese efecto. | Compara el estado antes y después. |
...
- La operación observable es: importar desde Perforce y enviar cambios de vuelta.
...
| --- | --- | --- |
| Caso base | importar desde Perforce y enviar cambios de vuelta | Ejecuta el ejemplo mínimo y registra el estado antes y después. |
| Alcance explícito | Aplicar git p4 a una referencia, rango o ruta identificada. | Resuelve cada argumento antes de ejecutar y usa `--` para rutas. |
| Validación | Comprobar el resultado de git p4 con una orden de lectura independiente. | No uses la misma salida como única prueba del cambio. |
